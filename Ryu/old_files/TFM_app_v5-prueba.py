...
class SimpleSwitchIGMP13(app_manager.RyuApp):
    ...
    def do_report(self, igmp_in, in_port, msg):
        ...
        if not self._to_hosts[dpid].get(server):
            #self._to_hosts[dpid][grp_addr]['servers'].setdefault(server, {'ports': {}})
            self._to_hosts[dpid].setdefault(server, {'ports': {}})
            #print(self._to_hosts)
            #print("New server")

        if not self._to_hosts[dpid][server]['ports'].get(in_port):
            self._to_hosts[dpid][server]['ports'][in_port] = {'out': False}
            #self._to_hosts[dpid][grp_addr][server]['ports'].setdefault(in_port, {'out': False, 'in': False})
            for port in self._to_hosts[dpid][server]['ports']:
                print(port)
        ...
        if not self._to_hosts[dpid][server]['ports'][in_port]['out']:
            self._to_hosts[dpid][server]['ports'][in_port]['out'] = True
            print(self._to_hosts)
            print("Flow added")
    ...
    def do_leave(self, igmp_in, in_port, msg):
        ...
        if self._to_hosts[dpid][server]['ports'][in_port]['out']:
            self._to_hosts[dpid][server]['ports'][in_port]['out'] = False
            #print(self._to_hosts)
            #print("Port by defect")

        if self._to_hosts[dpid][server]['ports'].get(in_port):
            print('Remove port')
            del self._to_hosts[dpid][server]['ports'][in_port]
            match = parser.OFPMatch(eth_type=0x0800, ip_proto=17, in_port=server)
            if len(self._to_hosts[dpid][server]['ports']) == 0:
        ...
        if len(self._to_hosts[dpid][server]['ports']) == 0:
            del self._to_hosts[dpid][server]['ports']
            print(self._to_hosts)

        if len(self._to_hosts[dpid][server]) == 0:
            print('Remove server')
            del self._to_hosts[dpid][server]
            print(self._to_hosts)
