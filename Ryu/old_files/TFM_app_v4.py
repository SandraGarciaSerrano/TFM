...
class SimpleSwitchIGMP13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.serv_to_clt = {'2': '6', '3': '6', '4': '6', '5': '7'}
    ...
    def _packet_in_handler(self, ev):
        ...
        if(igmp_in):    #Check if pkt is IGMP control
            ...
            if(igmp_in.msgtype==0x16):
                ...
                print(self.grp_to_mac[grp_addr])
                if in_port not in self.grp_to_mac[grp_addr]:
                    ...
                    if server not in port_server:
                    	...
                    	print(self.server_to_client[server])
                        #ports = self.grp_to_mac[grp_addr]
                    	port_clients_server = self.server_to_client[server]
                ...
                if in_port in self.grp_to_mac[grp_addr]:
                    ...
                    self.server_to_client[server].remove(in_port)
                    ...
                    if len(self.grp_to_mac[grp_addr]) == 0:
                        ...
                        del self.server_to_client[server]
                        del self.server_to_mac[grp_addr]
                        self.del_flow(datapath, 1, in_port, match, actions, msg.buffer_id)
                    elif len(self.server_to_client[server]) == 0:
                        print("Empty server")
                        del self.server_to_client[server]
                        self.server_to_mac[grp_addr].remove(server)
                        self.del_flow(datapath, 1, in_port, match, actions, msg.buffer_id)
                    else:
